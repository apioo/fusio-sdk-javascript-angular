...
export class AgentConsumerConnectionService implements Connection {
  ...
  async getChats(agentId: string, refId: number): Promise<ConsumerAgentMessageCollection> {
    return this.fusio.getClient().consumer().agent().message().getAll(agentId, refId);
  }
  ...
  async submit(agentId: string, refId: number, input: AgentInput): Promise<AgentOutput> {
    return this.fusio.getClient().consumer().agent().message().submit(agentId, input, refId);
  }
